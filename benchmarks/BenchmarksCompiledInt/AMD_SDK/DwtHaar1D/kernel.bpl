...
implementation {:source_name "dwtHaar1D"} {:kernel} $dwtHaar1D($tLevels: int, $signalLength: int, $levelsDone: int, $mLevels: int)
{
  ...
  $6:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b23 ==> _WRITE_HAS_OCCURRED_$$sharedArray ==> _WATCHED_OFFSET == local_id_x$1;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b22 ==> _READ_HAS_OCCURRED_$$sharedArray ==> _WATCHED_OFFSET == BV32_MUL(2, local_id_x$1) || _WATCHED_OFFSET == BV32_ADD(BV32_MUL(2, local_id_x$1), 1);
    assert {:tag "nowrite"} _b21 ==> !_WRITE_HAS_OCCURRED_$$sharedArray;
    assert {:tag "noread"} _b20 ==> !_READ_HAS_OCCURRED_$$sharedArray;
    assert {:tag "nowrite"} _b19 ==> !_WRITE_HAS_OCCURRED_$$coefsSignal;
    assert {:tag "pow2NotZero"} _b18 ==> $midOutPos.1$2 != 0;
    assert {:tag "pow2"} _b17 ==> $midOutPos.1$2 == 0 || BV32_AND($midOutPos.1$2, BV32_SUB($midOutPos.1$2, 1)) == 0;
    assert {:tag "pow2NotZero"} _b16 ==> $midOutPos.1$1 != 0;
    assert {:tag "pow2"} _b15 ==> $midOutPos.1$1 == 0 || BV32_AND($midOutPos.1$1, BV32_SUB($midOutPos.1$1, 1)) == 0;
    assert {:tag "pow2NotZero"} _b14 ==> $midOutPos.0$2 != 0;
    assert {:tag "pow2"} _b13 ==> $midOutPos.0$2 == 0 || BV32_AND($midOutPos.0$2, BV32_SUB($midOutPos.0$2, 1)) == 0;
    assert {:tag "pow2NotZero"} _b12 ==> $midOutPos.0$1 != 0;
    assert {:tag "pow2"} _b11 ==> $midOutPos.0$1 == 0 || BV32_AND($midOutPos.0$1, BV32_SUB($midOutPos.0$1, 1)) == 0;
    assert {:tag "pow2NotZero"} _b10 ==> $activeThreads.0 != 0;
    assert {:tag "pow2"} _b9 ==> $activeThreads.0 == 0 || BV32_AND($activeThreads.0, BV32_SUB($activeThreads.0, 1)) == 0;
    ...
    assert {:tag "loopBound"} {:thread 1} _b8 ==> BV32_UGE($activeThreads.0, BV32_SDIV(BV32_SHL(1, BV32_AND($0, 31)), 2));
    assert {:tag "loopBound"} {:thread 1} _b7 ==> BV32_ULE($activeThreads.0, BV32_SDIV(BV32_SHL(1, BV32_AND($0, 31)), 2));
    assert {:tag "loopBound"} {:thread 1} _b6 ==> BV32_SGE($activeThreads.0, BV32_SDIV(BV32_SHL(1, BV32_AND($0, 31)), 2));
    assert {:tag "loopBound"} {:thread 1} _b5 ==> BV32_SLE($activeThreads.0, BV32_SDIV(BV32_SHL(1, BV32_AND($0, 31)), 2));
