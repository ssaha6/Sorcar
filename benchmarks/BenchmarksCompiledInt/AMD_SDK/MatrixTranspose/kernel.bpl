...
procedure {:source_name "matrixTranspose"} {:kernel} $matrixTranspose($width: int, $height: int, $blockSize: int);
  requires {:sourceloc_num 0} {:thread 1} (if $blockSize == 16 then 1 else 0) != 0;
  requires {:sourceloc_num 1} {:thread 1} (if $width == 128 then 1 else 0) != 0;
  requires {:sourceloc_num 2} {:thread 1} (if $height == 128 then 1 else 0) != 0;
...
implementation {:source_name "matrixTranspose"} {:kernel} $matrixTranspose($width: int, $height: int, $blockSize: int)
{
  var v0$1: int;
  var v0$2: int;
  var v1$1: int;
