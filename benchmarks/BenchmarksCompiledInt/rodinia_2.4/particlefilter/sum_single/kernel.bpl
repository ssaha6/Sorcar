...
implementation {:source_name "sum_kernel"} {:kernel} $sum_kernel($Nparticles: int)
{
  ...
  $2:
    ...
    assert {:tag "disabledMaintainsInstrumentation"} _b9 ==> !p0$1 ==> _READ_HAS_OCCURRED_$$partial_sums$ghost$$2 == _READ_HAS_OCCURRED_$$partial_sums;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b8 ==> _READ_HAS_OCCURRED_$$partial_sums ==> _WATCHED_OFFSET mod 1 == 0 mod 1;
    assume {:predicate "p1"} {:dominator_predicate "p0"} true;
    assert {:do_not_predicate} {:tag "accessOnlyIfEnabledInEnclosingScopes"} {:thread 1} _b7 ==> _READ_HAS_OCCURRED_$$partial_sums ==> BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) == 0;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b6 ==> BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) == 0 && BV32_SLT($x.0$1, BV32_ADD(BV32_UDIV($Nparticles, group_size_x), 1)) ==> p1$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b6 ==> BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2) == 0 && BV32_SLT($x.0$2, BV32_ADD(BV32_UDIV($Nparticles, group_size_x), 1)) ==> p1$2;
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 1} p1$1 ==> _b5 ==> p1$1 ==> BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) == 0;
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 2} p1$2 ==> _b5 ==> p1$2 ==> BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2) == 0;
    assert {:tag "loopBound"} {:thread 1} p1$1 ==> _b4 ==> BV32_UGE($x.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p1$2 ==> _b4 ==> BV32_UGE($x.0$2, 0);
    assert {:tag "loopBound"} {:thread 1} p1$1 ==> _b3 ==> BV32_ULE($x.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p1$2 ==> _b3 ==> BV32_ULE($x.0$2, 0);
    assert {:tag "loopBound"} {:thread 1} p1$1 ==> _b2 ==> BV32_SGE($x.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p1$2 ==> _b2 ==> BV32_SGE($x.0$2, 0);
    assert {:tag "loopBound"} {:thread 1} p1$1 ==> _b1 ==> BV32_SLE($x.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p1$2 ==> _b1 ==> BV32_SLE($x.0$2, 0);
    assert {:tag "loopCounterIsStrided"} {:thread 1} p1$1 ==> _b0 ==> $x.0$1 mod 1 == 0 mod 1;
    assert {:tag "loopCounterIsStrided"} {:thread 2} p1$2 ==> _b0 ==> $x.0$2 mod 1 == 0 mod 1;
