...
implementation {:source_name "generatePaths<float>"} {:kernel} $_Z13generatePathsIfEvPT_P17curandStateXORWOWPK11AsianOptionIS0_Ejj($numSims: int, $numTimesteps: int)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b15 ==> _WRITE_HAS_OCCURRED_$$paths ==> _WATCHED_OFFSET mod $numSims == $i.0$1 mod $numSims;
    ...
    assert {:tag "loopCounterIsStrided"} {:thread 1} p0$1 ==> _b0 ==> $i.0$1 mod BV32_MUL(num_groups_x, group_size_x) == v0$1 mod BV32_MUL(num_groups_x, group_size_x);
    assert {:tag "loopCounterIsStrided"} {:thread 2} p0$2 ==> _b0 ==> $i.0$2 mod BV32_MUL(num_groups_x, group_size_x) == v0$2 mod BV32_MUL(num_groups_x, group_size_x);
  ...
  $3:
    ...
    assert {:tag "disabledMaintainsInstrumentation"} _b17 ==> !p1$1 ==> _WRITE_HAS_OCCURRED_$$paths$ghost$$3 == _WRITE_HAS_OCCURRED_$$paths;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b16 ==> _WRITE_HAS_OCCURRED_$$paths ==> _WATCHED_OFFSET mod $numSims == $i.0$1 mod $numSims;
    assume {:predicate "p2"} {:dominator_predicate "p1"} true;
    assert p2$1 ==> p0$1;
    assert p2$2 ==> p0$2;
    assert {:do_not_predicate} {:tag "accessOnlyIfEnabledInEnclosingScopes"} {:thread 1} _b14 ==> _WRITE_HAS_OCCURRED_$$paths ==> BV32_ULT($i.0$1, $numSims);
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b13 ==> BV32_ULT($i.0$1, $numSims) && BV32_ULT($t.0$1, $numTimesteps) ==> p2$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b13 ==> BV32_ULT($i.0$2, $numSims) && BV32_ULT($t.0$2, $numTimesteps) ==> p2$2;
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 1} p2$1 ==> _b12 ==> p2$1 ==> BV32_ULT($i.0$1, $numSims);
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 2} p2$2 ==> _b12 ==> p2$2 ==> BV32_ULT($i.0$2, $numSims);
    assert {:tag "loopBound"} {:thread 1} p2$1 ==> _b11 ==> BV32_UGE($t.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p2$2 ==> _b11 ==> BV32_UGE($t.0$2, 0);
    assert {:tag "loopBound"} {:thread 1} p2$1 ==> _b10 ==> BV32_ULE($t.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p2$2 ==> _b10 ==> BV32_ULE($t.0$2, 0);
    assert {:tag "loopBound"} {:thread 1} p2$1 ==> _b9 ==> BV32_SGE($t.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p2$2 ==> _b9 ==> BV32_SGE($t.0$2, 0);
    assert {:tag "loopBound"} {:thread 1} p2$1 ==> _b8 ==> BV32_SLE($t.0$1, 0);
    assert {:tag "loopBound"} {:thread 2} p2$2 ==> _b8 ==> BV32_SLE($t.0$2, 0);
    assert {:tag "loopCounterIsStrided"} {:thread 1} p2$1 ==> _b7 ==> $t.0$1 mod 1 == 0 mod 1;
    assert {:tag "loopCounterIsStrided"} {:thread 2} p2$2 ==> _b7 ==> $t.0$2 mod 1 == 0 mod 1;
    assert {:tag "loopCounterIsStrided"} {:thread 1} p2$1 ==> _b6 ==> $output.0$1 mod $numSims == $i.0$1 mod $numSims;
    assert {:tag "loopCounterIsStrided"} {:thread 2} p2$2 ==> _b6 ==> $output.0$2 mod $numSims == $i.0$2 mod $numSims;
