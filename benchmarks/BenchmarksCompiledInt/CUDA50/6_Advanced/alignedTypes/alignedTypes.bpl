...
implementation {:source_name "testKernel<int>"} {:kernel} $_Z10testKernelIiEvPT_S1_i($numElements: int)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b6 ==> _WRITE_HAS_OCCURRED_$$d_odata ==> _WATCHED_OFFSET mod BV32_MUL(group_size_x, num_groups_x) == BV32_ADD(BV32_MUL(group_size_x, group_id_x$1), local_id_x$1) mod BV32_MUL(group_size_x, num_groups_x);
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b5 ==> BV32_SLT($pos.0$1, $numElements) ==> p0$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b5 ==> BV32_SLT($pos.0$2, $numElements) ==> p0$2;
    ...
    assert {:tag "loopCounterIsStrided"} {:thread 1} p0$1 ==> _b0 ==> $pos.0$1 mod BV32_MUL(group_size_x, num_groups_x) == BV32_ADD(BV32_MUL(group_size_x, group_id_x$1), local_id_x$1) mod BV32_MUL(group_size_x, num_groups_x);
    assert {:tag "loopCounterIsStrided"} {:thread 2} p0$2 ==> _b0 ==> $pos.0$2 mod BV32_MUL(group_size_x, num_groups_x) == BV32_ADD(BV32_MUL(group_size_x, group_id_x$2), local_id_x$2) mod BV32_MUL(group_size_x, num_groups_x);
