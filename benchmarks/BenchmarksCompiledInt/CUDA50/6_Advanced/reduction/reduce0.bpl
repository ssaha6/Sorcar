...
procedure {:source_name "reduce0<int>"} {:kernel} $_Z7reduce0IiEvPT_S1_j($n: int);
...
implementation {:source_name "reduce0<int>"} {:kernel} $_Z7reduce0IiEvPT_S1_j($n: int)
{
  var $0$1: int;
  var $0$2: int;
  var $s.0: int;
  ...
  $4:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b8 ==> _READ_HAS_OCCURRED_$$__smem ==> _WATCHED_OFFSET mod 1 == BV32_ADD(0, local_id_x$1) mod 1 || _WATCHED_OFFSET == local_id_x$1;
    ...
    assert {:tag "loopBound"} {:thread 1} _b3 ==> BV32_UGE($s.0, 1);
    assert {:tag "loopBound"} {:thread 1} _b2 ==> BV32_ULE($s.0, 1);
    assert {:tag "loopBound"} {:thread 1} _b1 ==> BV32_SGE($s.0, 1);
    assert {:tag "loopBound"} {:thread 1} _b0 ==> BV32_SLE($s.0, 1);
  ...
  $falsebb0:
    ...
    call {:sourceloc} {:sourceloc_num 18} _LOG_READ_$$__smem(p4$1, 0, $$__smem[1bv1][0]);
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 18} true;
    call {:check_id "check_state_1"} {:sourceloc} {:sourceloc_num 18} _CHECK_READ_$$__smem(p4$2, 0, $$__smem[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][0]);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_READ_$$__smem"} true;
    v8$1 := (if p4$1 then $$__smem[1bv1][0] else v8$1);
    v8$2 := (if p4$2 then $$__smem[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][0] else v8$2);
  ...
  __partitioned_block_$truebb0_0:
    assume {:partition} v3;
    v4$1 := BV32_UREM(local_id_x$1, BV32_MUL(2, $s.0)) == 0;
    v4$2 := BV32_UREM(local_id_x$2, BV32_MUL(2, $s.0)) == 0;
