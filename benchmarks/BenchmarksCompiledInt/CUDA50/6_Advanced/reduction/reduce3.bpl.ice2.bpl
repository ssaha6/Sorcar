...
assert  my_inv (  (  BV32_SLE($s.0, BV32_UDIV(group_size_x, 2)) ) ,  (  BV32_SGE($s.0, BV32_UDIV(group_size_x, 2)) ) ,  (  BV32_ULE($s.0, BV32_UDIV(group_size_x, 2)) ) ,  (  BV32_UGE($s.0, BV32_UDIV(group_size_x, 2)) ) ,  (  $s.0 == 0 || BV32_AND($s.0, BV32_SUB($s.0, 1)) == 0 ) ,  (  $s.0 != 0 ) ,  (  !_READ_HAS_OCCURRED_$$__smem ) ,  (  !_WRITE_HAS_OCCURRED_$$__smem ) ,  (  _READ_HAS_OCCURRED_$$__smem ==> _WATCHED_OFFSET mod 1 == BV32_ADD(0, local_id_x$1) mod 1 ) ,  (  _WRITE_HAS_OCCURRED_$$__smem ==> _WATCHED_OFFSET == local_id_x$1 )  ); 
