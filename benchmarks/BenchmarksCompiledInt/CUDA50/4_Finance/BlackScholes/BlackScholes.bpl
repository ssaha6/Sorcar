...
implementation {:source_name "BlackScholesGPU"} {:kernel} $_Z15BlackScholesGPUPfS_S_S_S_ffi($Riskfree: int, $Volatility: int, $optN: int)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b7 ==> _WRITE_HAS_OCCURRED_$$d_PutResult ==> _WATCHED_OFFSET mod BV32_MUL(group_size_x, num_groups_x) == v0$1 mod BV32_MUL(group_size_x, num_groups_x);
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b6 ==> _WRITE_HAS_OCCURRED_$$d_CallResult ==> _WATCHED_OFFSET mod BV32_MUL(group_size_x, num_groups_x) == v0$1 mod BV32_MUL(group_size_x, num_groups_x);
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b5 ==> BV32_SLT($opt.0$1, $optN) ==> p0$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b5 ==> BV32_SLT($opt.0$2, $optN) ==> p0$2;
    assert {:tag "loopBound"} {:thread 1} p0$1 ==> _b4 ==> BV32_UGE($opt.0$1, v0$1);
    assert {:tag "loopBound"} {:thread 2} p0$2 ==> _b4 ==> BV32_UGE($opt.0$2, v0$2);
    assert {:tag "loopBound"} {:thread 1} p0$1 ==> _b3 ==> BV32_ULE($opt.0$1, v0$1);
    assert {:tag "loopBound"} {:thread 2} p0$2 ==> _b3 ==> BV32_ULE($opt.0$2, v0$2);
    assert {:tag "loopBound"} {:thread 1} p0$1 ==> _b2 ==> BV32_SGE($opt.0$1, v0$1);
    assert {:tag "loopBound"} {:thread 2} p0$2 ==> _b2 ==> BV32_SGE($opt.0$2, v0$2);
    assert {:tag "loopBound"} {:thread 1} p0$1 ==> _b1 ==> BV32_SLE($opt.0$1, v0$1);
    assert {:tag "loopBound"} {:thread 2} p0$2 ==> _b1 ==> BV32_SLE($opt.0$2, v0$2);
    assert {:tag "loopCounterIsStrided"} {:thread 1} p0$1 ==> _b0 ==> $opt.0$1 mod BV32_MUL(group_size_x, num_groups_x) == v0$1 mod BV32_MUL(group_size_x, num_groups_x);
    assert {:tag "loopCounterIsStrided"} {:thread 2} p0$2 ==> _b0 ==> $opt.0$2 mod BV32_MUL(group_size_x, num_groups_x) == v0$2 mod BV32_MUL(group_size_x, num_groups_x);
