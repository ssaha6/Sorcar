...
implementation {:source_name "box_muller_kernel"} {:kernel} $_Z17box_muller_kernelPfS_i($n_per_RNG: int)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b9 ==> _WRITE_HAS_OCCURRED_$$normalized_random_nums ==> _WATCHED_OFFSET mod BV32_MUL(2, 4096) == BV32_ADD(BV32_MUL(0, 4096), BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)) mod BV32_MUL(2, 4096) || _WATCHED_OFFSET mod BV32_MUL(2, 4096) == BV32_ADD(BV32_MUL(BV32_ADD(0, 1), 4096), BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)) mod BV32_MUL(2, 4096);
    assert {:tag "accessUpperBoundBlock"} _b8 ==> _WRITE_HAS_OCCURRED_$$normalized_random_nums ==> BV32_SLT(_WATCHED_OFFSET, BV32_ADD(BV32_MUL(BV32_ADD(group_id_x$1, 1), group_size_x), local_id_x$1));
    assert {:tag "accessLowerBoundBlock"} _b7 ==> _WRITE_HAS_OCCURRED_$$normalized_random_nums ==> BV32_SLE(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), _WATCHED_OFFSET);
    assert {:tag "accessUpperBoundBlock"} _b6 ==> _WRITE_HAS_OCCURRED_$$normalized_random_nums ==> BV32_SLT(_WATCHED_OFFSET, BV32_ADD(BV32_MUL(BV32_ADD(group_id_x$1, 1), group_size_x), local_id_x$1));
    assert {:tag "accessLowerBoundBlock"} _b5 ==> _WRITE_HAS_OCCURRED_$$normalized_random_nums ==> BV32_SLE(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), _WATCHED_OFFSET);
    assert {:tag "loopBound"} {:thread 1} _b4 ==> BV32_UGE($out.0, 0);
    assert {:tag "loopBound"} {:thread 1} _b3 ==> BV32_ULE($out.0, 0);
    assert {:tag "loopBound"} {:thread 1} _b2 ==> BV32_SGE($out.0, 0);
    assert {:tag "loopBound"} {:thread 1} _b1 ==> BV32_SLE($out.0, 0);
    assert {:tag "loopCounterIsStrided"} {:thread 1} _b0 ==> $out.0 mod 2 == 0 mod 2;
