...
implementation {:source_name "spmv_csr_scalar_kernel"} {:kernel} $spmv_csr_scalar_kernel($dim: int)
{
  var $t.0$1: int;
  var $t.0$2: int;
  var $j.0$1: int;
  ...
  $0:
    ...
    v3$1 := (if p0$1 then $$rowDelimiters[BV32_ADD(v0$1, 1)] else v3$1);
    v3$2 := (if p0$2 then $$rowDelimiters[BV32_ADD(v0$2, 1)] else v3$2);
    $t.0$1, $j.0$1 := (if p0$1 then 0 else $t.0$1), (if p0$1 then v2$1 else $j.0$1);
    $t.0$2, $j.0$2 := (if p0$2 then 0 else $t.0$2), (if p0$2 then v2$2 else $j.0$2);
  ...
  $2:
    ...
    $t.0$1, $j.0$1 := (if p2$1 then FADD32($t.0$1, FMUL32(v6$1, v7$1)) else $t.0$1), (if p2$1 then BV32_ADD($j.0$1, 1) else $j.0$1);
    $t.0$2, $j.0$2 := (if p2$2 then FADD32($t.0$2, FMUL32(v6$2, v7$2)) else $t.0$2), (if p2$2 then BV32_ADD($j.0$2, 1) else $j.0$2);
