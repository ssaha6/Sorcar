...
implementation {:source_name "readRand"} {:kernel} $readRand($n: int, $samp: int, $w: int, $h: int)
{
  var $ridx.0$1: int;
  var $ridx.0$2: int;
  var $sum.0$1: int;
  ...
  $0:
    ...
    $ridx.0$1, $sum.0$1, $i.0 := BV_CONCAT(v1$1, v0$1), 0, 0;
    $ridx.0$2, $sum.0$2 := BV_CONCAT(v1$2, v0$2), 0;
  ...
  $truebb:
    assume {:partition} v3;
    v4$1 := BV32_SLT(BV_EXTRACT($ridx.0$1, 32, 0), 0);
    v4$2 := BV32_SLT(BV_EXTRACT($ridx.0$2, 32, 0), 0);
    ...
    v5$1 := (if p1$1 then BV32_SGE(BV_EXTRACT($ridx.0$1, 32, 0), 8192) else v5$1);
    v5$2 := (if p1$2 then BV32_SGE(BV_EXTRACT($ridx.0$2, 32, 0), 8192) else v5$2);
    ...
    $1$1 := (if p2$1 then BV_EXTRACT($ridx.0$1, 32, 0) else $1$1);
    $1$2 := (if p2$2 then BV_EXTRACT($ridx.0$2, 32, 0) else $1$2);
    $1$1 := (if p3$1 then 8191 else $1$1);
    $1$2 := (if p3$2 then 8191 else $1$2);
    $0$1 := (if p1$1 then $1$1 else $0$1);
    $0$2 := (if p1$2 then $1$2 else $0$2);
    v6$1 := BV32_SLT(BV_EXTRACT($ridx.0$1, 64, 32), 0);
    v6$2 := BV32_SLT(BV_EXTRACT($ridx.0$2, 64, 32), 0);
    ...
    v7$1 := (if p5$1 then BV32_SGE(BV_EXTRACT($ridx.0$1, 64, 32), 8192) else v7$1);
    v7$2 := (if p5$2 then BV32_SGE(BV_EXTRACT($ridx.0$2, 64, 32), 8192) else v7$2);
    ...
    $3$1 := (if p6$1 then BV_EXTRACT($ridx.0$1, 64, 32) else $3$1);
    $3$2 := (if p6$2 then BV_EXTRACT($ridx.0$2, 64, 32) else $3$2);
    ...
    havoc v10$1, v10$2;
    havoc v11$1, v11$2;
    v12$1 := BV_EXTRACT($ridx.0$1, 64, 32);
    v12$2 := BV_EXTRACT($ridx.0$2, 64, 32);
    $ridx.0$1, $sum.0$1, $i.0 := BV_CONCAT(BV32_AND(BV32_ADD(BV32_MUL(v12$1, 5), 11), BV32_SUB($h, 1)), BV32_AND(BV32_ADD(BV32_MUL(BV_EXTRACT($ridx.0$1, 32, 0), 3), 29), BV32_SUB($w, 1))), FADD32($sum.0$1, v8$1), BV32_ADD($i.0, 1);
    $ridx.0$2, $sum.0$2 := BV_CONCAT(BV32_AND(BV32_ADD(BV32_MUL(v12$2, 5), 11), BV32_SUB($h, 1)), BV32_AND(BV32_ADD(BV32_MUL(BV_EXTRACT($ridx.0$2, 32, 0), 3), 29), BV32_SUB($w, 1))), FADD32($sum.0$2, v8$2);
