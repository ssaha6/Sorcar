...
procedure {:source_name "kernel1"} {:kernel} $kernel1($n: int, $m: int);
  requires {:sourceloc_num 0} {:thread 1} (if $n == 2048 then 1 else 0) != 0;
  requires {:sourceloc_num 1} {:thread 1} (if $m == 2048 then 1 else 0) != 0;
  requires {:sourceloc_num 2} {:thread 1} (if BV32_AND(BV32_AND(BV32_AND(BV1_ZEXT32((if BV32_SGE($n, 1) then 1 else 0)), BV1_ZEXT32((if BV32_SLE($n, 2147483647) then 1 else 0))), BV1_ZEXT32((if BV32_SLE($m, 2147483647) then 1 else 0))), BV1_ZEXT32((if BV64_SGE(BV32_SEXT64($m), -2147483648) then 1 else 0))) != 0 then 1 else 0) != 0;
...
implementation {:source_name "kernel1"} {:kernel} $kernel1($n: int, $m: int)
{
  var $c0.0$1: int;
  var $c0.0$2: int;
  var v0$1: int;
...
assert  my_inv (  ( p0$1 ==> $c0.0$1 mod 1048576 == BV64_MUL(32, v0$1) mod 1048576 )  && ( p0$2 ==> $c0.0$2 mod 1048576 == BV64_MUL(32, v0$2) mod 1048576 ) ,  ( p0$1 ==> BV32_SLE($c0.0$1, BV64_MUL(32, v0$1)) )  && ( p0$2 ==> BV32_SLE($c0.0$2, BV64_MUL(32, v0$2)) ) ,  ( p0$1 ==> BV32_SGE($c0.0$1, BV64_MUL(32, v0$1)) )  && ( p0$2 ==> BV32_SGE($c0.0$2, BV64_MUL(32, v0$2)) ) ,  ( p0$1 ==> BV32_ULE($c0.0$1, BV64_MUL(32, v0$1)) )  && ( p0$2 ==> BV32_ULE($c0.0$2, BV64_MUL(32, v0$2)) ) ,  ( p0$1 ==> BV32_UGE($c0.0$1, BV64_MUL(32, v0$1)) )  && ( p0$2 ==> BV32_UGE($c0.0$2, BV64_MUL(32, v0$2)) ) ,  (  BV64_SLT($c0.0$1, BV32_SEXT64($n)) ==> p0$1 )  && (  BV64_SLT($c0.0$2, BV32_SEXT64($n)) ==> p0$2 ) ,  (  _WRITE_HAS_OCCURRED_$$y ==> _WATCHED_OFFSET mod 1 == 0 mod 1 )  ); 
    ...
    call {:sourceloc} {:sourceloc_num 8} _LOG_WRITE_$$y(p3$1, BV_EXTRACT(BV64_ADD(v1$1, $c0.0$1), 32, 0), 0, $$y[BV_EXTRACT(BV64_ADD(v1$1, $c0.0$1), 32, 0)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$y(p3$2, BV_EXTRACT(BV64_ADD(v1$2, $c0.0$2), 32, 0));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 8} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 8} _CHECK_WRITE_$$y(p3$2, BV_EXTRACT(BV64_ADD(v1$2, $c0.0$2), 32, 0), 0);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$y"} true;
    $$y[BV_EXTRACT(BV64_ADD(v1$1, $c0.0$1), 32, 0)] := (if p3$1 then 0 else $$y[BV_EXTRACT(BV64_ADD(v1$1, $c0.0$1), 32, 0)]);
    $$y[BV_EXTRACT(BV64_ADD(v1$2, $c0.0$2), 32, 0)] := (if p3$2 then 0 else $$y[BV_EXTRACT(BV64_ADD(v1$2, $c0.0$2), 32, 0)]);
