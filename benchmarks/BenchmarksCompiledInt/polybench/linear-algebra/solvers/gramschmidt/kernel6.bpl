...
implementation {:source_name "kernel6"} {:kernel} $kernel6($n: int, $m: int, $c0: int)
{
  ...
  $3:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b7 ==> _READ_HAS_OCCURRED_$$kernel6.shared_R ==> _WATCHED_OFFSET == 0;
    ...
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b5 ==> BV64_SLT($c1.0$1, BV32_SEXT64($m)) ==> p2$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b5 ==> BV64_SLT($c1.0$2, BV32_SEXT64($m)) ==> p2$2;
