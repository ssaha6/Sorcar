...
procedure {:source_name "kernel0"} {:kernel} $kernel0($n: int);
  ...
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _READ_HAS_OCCURRED_$$r ==> BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1)) == 0 then 1 else 0) != 0;
...
implementation {:source_name "kernel0"} {:kernel} $kernel0($n: int)
{
  ...
  $0:
    havoc v0$1, v0$2;
    call {:sourceloc} {:sourceloc_num 5} _LOG_WRITE_$$alpha(true, 0, FSUB64(-9223372036854775808, v0$1), $$alpha[0]);
    ...
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 5} _CHECK_WRITE_$$alpha(true, 0, FSUB64(-9223372036854775808, v0$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$alpha"} true;
    $$alpha[0] := FSUB64(-9223372036854775808, v0$1);
    $$alpha[0] := FSUB64(-9223372036854775808, v0$2);
    return;
}
