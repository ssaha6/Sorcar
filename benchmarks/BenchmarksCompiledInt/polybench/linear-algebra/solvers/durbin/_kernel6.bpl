...
procedure {:source_name "kernel6"} {:kernel} $kernel6($n: int, $c0: int, $c1: int);
  ...
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _READ_HAS_OCCURRED_$$r ==> BV64_ADD(BV64_ADD(BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1))), $c1), 1) == $c0 then 1 else 0) != 0;
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 3} {:thread 1} (if _READ_HAS_OCCURRED_$$y ==> $c1 == BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1))) then 1 else 0) != 0;
...
implementation {:source_name "kernel6"} {:kernel} $kernel6($n: int, $c0: int, $c1: int)
{
  var v0$1: int;
  var v0$2: int;
  var v1$1: int;
  ...
  $0:
    havoc v0$1, v0$2;
    havoc v1$1, v1$2;
    call {:sourceloc} {:sourceloc_num 7} _LOG_READ_$$sum(true, 0, $$sum[0]);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 7} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 7} _CHECK_READ_$$sum(true, 0, $$sum[0]);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_READ_$$sum"} true;
    v2$1 := $$sum[0];
    v2$2 := $$sum[0];
    call {:sourceloc} {:sourceloc_num 8} _LOG_WRITE_$$sum(true, 0, FADD64(v2$1, FMUL64(v0$1, v1$1)), $$sum[0]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$sum(true, 0);
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 8} true;
    call {:check_id "check_state_1"} {:sourceloc} {:sourceloc_num 8} _CHECK_WRITE_$$sum(true, 0, FADD64(v2$2, FMUL64(v0$2, v1$2)));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$sum"} true;
    $$sum[0] := FADD64(v2$1, FMUL64(v0$1, v1$1));
    $$sum[0] := FADD64(v2$2, FMUL64(v0$2, v1$2));
    return;
}
