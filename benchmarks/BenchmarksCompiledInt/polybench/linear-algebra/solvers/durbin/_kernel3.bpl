...
procedure {:source_name "kernel3"} {:kernel} $kernel3($n: int);
  requires {:sourceloc_num 0} {:thread 1} (if $n == 1024 then 1 else 0) != 0;
  requires {:sourceloc_num 1} {:thread 1} (if BV32_AND(BV1_ZEXT32((if BV32_SLE($n, 2147483647) then 1 else 0)), BV1_ZEXT32((if BV64_SGE(BV32_SEXT64($n), -2147483648) then 1 else 0))) != 0 then 1 else 0) != 0;
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _WRITE_HAS_OCCURRED_$$y ==> BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1)) == 0 then 1 else 0) != 0;
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 3} {:thread 1} (if _READ_HAS_OCCURRED_$$r ==> BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1)) == 0 then 1 else 0) != 0;
...
implementation {:source_name "kernel3"} {:kernel} $kernel3($n: int)
{
  var v0$1: int;
  var v0$2: int;
  ...
  $0:
    havoc v0$1, v0$2;
    call {:sourceloc} {:sourceloc_num 6} _LOG_WRITE_$$y(true, 0, FSUB64(-9223372036854775808, v0$1), $$y[0]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$y(true, 0);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 6} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 6} _CHECK_WRITE_$$y(true, 0, FSUB64(-9223372036854775808, v0$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$y"} true;
    $$y[0] := FSUB64(-9223372036854775808, v0$1);
    $$y[0] := FSUB64(-9223372036854775808, v0$2);
    return;
}
