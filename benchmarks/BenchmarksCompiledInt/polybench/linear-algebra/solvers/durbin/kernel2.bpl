...
implementation {:source_name "kernel2"} {:kernel} $kernel2($n: int, $c0: int)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b6 ==> _WRITE_HAS_OCCURRED_$$y ==> _WATCHED_OFFSET mod 1 == 0 mod 1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b5 ==> BV64_SLT($c1.0$1, BV64_SUB($c0, 1)) ==> p0$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b5 ==> BV64_SLT($c1.0$2, BV64_SUB($c0, 1)) ==> p0$2;
    ...
    assert {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _WRITE_HAS_OCCURRED_$$y ==> BV32_AND(BV32_AND(BV32_AND(BV32_AND(BV1_ZEXT32((if BV64_SGE(BV64_ADD(BV64_MUL(32, BV32_ZEXT64(group_id_x$1)), BV32_ZEXT64(local_id_x$1)), 0) then 1 else 0)), BV1_ZEXT32((if BV64_SLE(BV64_ADD(BV64_MUL(32, BV32_ZEXT64(group_id_x$1)), BV32_ZEXT64(local_id_x$1)), 1048575) then 1 else 0))), BV1_ZEXT32((if BV32_UGE(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1)), 0) then 1 else 0))), BV1_ZEXT32((if BV64_SGE($c0, BV32_ZEXT64(BV32_ADD(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1)), 2))) then 1 else 0))), BV1_ZEXT32((if BV64_SREM(BV64_SUB(BV64_ADD(BV64_MUL(32, BV32_ZEXT64(group_id_x$1)), BV32_ZEXT64(local_id_x$1)), BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1)))), 1048576) == 0 then 1 else 0))) != 0 then 1 else 0) != 0;
    assert {:block_sourceloc} {:sourceloc_num 5} p0$1 ==> true;
    v2$1 := (if p0$1 then BV64_SLT($c1.0$1, BV64_SUB($c0, 1)) else v2$1);
    v2$2 := (if p0$2 then BV64_SLT($c1.0$2, BV64_SUB($c0, 1)) else v2$2);
    p1$1 := false;
    p1$2 := false;
    p2$1 := false;
    p2$2 := false;
    ...
    v3$1 := (if p1$1 then BV64_SGE($c0, BV64_ADD(BV64_ADD(v1$1, $c1.0$1), 2)) else v3$1);
    v3$2 := (if p1$2 then BV64_SGE($c0, BV64_ADD(BV64_ADD(v1$2, $c1.0$2), 2)) else v3$2);
    ...
    call {:sourceloc} {:sourceloc_num 9} _LOG_WRITE_$$y(p3$1, BV_EXTRACT(BV64_ADD(v1$1, $c1.0$1), 32, 0), v4$1, $$y[BV_EXTRACT(BV64_ADD(v1$1, $c1.0$1), 32, 0)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$y(p3$2, BV_EXTRACT(BV64_ADD(v1$2, $c1.0$2), 32, 0));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 9} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 9} _CHECK_WRITE_$$y(p3$2, BV_EXTRACT(BV64_ADD(v1$2, $c1.0$2), 32, 0), v4$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$y"} true;
    $$y[BV_EXTRACT(BV64_ADD(v1$1, $c1.0$1), 32, 0)] := (if p3$1 then v4$1 else $$y[BV_EXTRACT(BV64_ADD(v1$1, $c1.0$1), 32, 0)]);
    $$y[BV_EXTRACT(BV64_ADD(v1$2, $c1.0$2), 32, 0)] := (if p3$2 then v4$2 else $$y[BV_EXTRACT(BV64_ADD(v1$2, $c1.0$2), 32, 0)]);
