...
procedure {:source_name "kernel8"} {:kernel} $kernel8($n: int, $c0: int);
  ...
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _WRITE_HAS_OCCURRED_$$y ==> $c0 == BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), (if BV32_SGE($n, 2) then $n else 1))) then 1 else 0) != 0;
...
implementation {:source_name "kernel8"} {:kernel} $kernel8($n: int, $c0: int)
{
  var v0$1: int;
  var v0$2: int;
  ...
  $0:
    havoc v0$1, v0$2;
    call {:sourceloc} {:sourceloc_num 5} _LOG_WRITE_$$y(true, BV_EXTRACT($c0, 32, 0), v0$1, $$y[BV_EXTRACT($c0, 32, 0)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$y(true, BV_EXTRACT($c0, 32, 0));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 5} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 5} _CHECK_WRITE_$$y(true, BV_EXTRACT($c0, 32, 0), v0$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$y"} true;
    $$y[BV_EXTRACT($c0, 32, 0)] := v0$1;
    $$y[BV_EXTRACT($c0, 32, 0)] := v0$2;
    return;
}
