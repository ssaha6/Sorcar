...
implementation {:source_name "kernel5"} {:kernel} $kernel5($n: int, $c0: int)
{
  ...
  $10:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b6 ==> _READ_HAS_OCCURRED_$$A ==> _WATCHED_OFFSET mod 1 == 0 mod 1 || _WATCHED_OFFSET mod 1 == 0 mod 1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b5 ==> BV64_SLE($c1.0$1, BV64_SDIV(BV64_SUB($c0, 1), 3)) ==> p6$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b5 ==> BV64_SLE($c1.0$2, BV64_SDIV(BV64_SUB($c0, 1), 3)) ==> p6$2;
