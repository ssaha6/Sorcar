...
procedure {:source_name "kernel8"} {:kernel} $kernel8($n: int, $c0: int, $c1: int);
  requires {:sourceloc_num 0} {:thread 1} (if $n == 256 then 1 else 0) != 0;
  requires {:sourceloc_num 1} {:thread 1} (if BV32_AND(BV32_AND(BV32_AND(BV1_ZEXT32((if BV32_SLE($n, 2147483647) then 1 else 0)), BV1_ZEXT32((if BV64_SLE($c0, 0) then 1 else 0))), BV1_ZEXT32((if BV64_SGE(BV64_ADD($c0, $c1), 1) then 1 else 0))), BV1_ZEXT32((if BV64_SGE(BV32_SEXT64($n), BV64_ADD($c1, 1)) then 1 else 0))) != 0 then 1 else 0) != 0;
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _READ_HAS_OCCURRED_$$A ==> BV32_AND(BV1_ZEXT32((if BV64_ADD(BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), $n), $n)), $c0) == 0 then 1 else 0)), BV1_ZEXT32((if $c1 == BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), $n)) then 1 else 0))) != 0 then 1 else 0) != 0;
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 3} {:thread 1} (if _READ_HAS_OCCURRED_$$x ==> $c1 == BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), $n)) then 1 else 0) != 0;
...
implementation {:source_name "kernel8"} {:kernel} $kernel8($n: int, $c0: int, $c1: int)
{
  var v0$1: int;
  var v0$2: int;
  var v1$1: int;
  ...
  $0:
    havoc v0$1, v0$2;
    havoc v1$1, v1$2;
    call {:sourceloc} {:sourceloc_num 7} _LOG_READ_$$w(true, 0, $$w[0]);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 7} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 7} _CHECK_READ_$$w(true, 0, $$w[0]);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_READ_$$w"} true;
    v2$1 := $$w[0];
    v2$2 := $$w[0];
    call {:sourceloc} {:sourceloc_num 8} _LOG_WRITE_$$w(true, 0, FSUB64(v2$1, FMUL64(v0$1, v1$1)), $$w[0]);
    ...
    call {:check_id "check_state_1"} {:sourceloc} {:sourceloc_num 8} _CHECK_WRITE_$$w(true, 0, FSUB64(v2$2, FMUL64(v0$2, v1$2)));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$w"} true;
    $$w[0] := FSUB64(v2$1, FMUL64(v0$1, v1$1));
    $$w[0] := FSUB64(v2$2, FMUL64(v0$2, v1$2));
    return;
}
