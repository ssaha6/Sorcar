...
procedure {:source_name "kernel6"} {:kernel} $kernel6($n: int, $c0: int);
  ...
  requires {:procedure_wide_invariant} {:do_not_predicate} {:sourceloc_num 2} {:thread 1} (if _WRITE_HAS_OCCURRED_$$y ==> $c0 == BV32_ZEXT64(BV32_UREM(BV32_UDIV(BV32_MUL(8, _WATCHED_OFFSET), 8), $n)) then 1 else 0) != 0;
...
implementation {:source_name "kernel6"} {:kernel} $kernel6($n: int, $c0: int)
{
  var v0$1: int;
  var v0$2: int;
