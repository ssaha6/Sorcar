...
implementation {:source_name "kernel3"} {:kernel} $kernel3($float_n: int, $m: int, $n: int)
{
  var $c0.0$1: int;
  var $c0.0$2: int;
  var v0$1: int;
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b7 ==> _WRITE_HAS_OCCURRED_$$stddev ==> _WATCHED_OFFSET mod 1 == 0 mod 1;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b6 ==> _READ_HAS_OCCURRED_$$stddev ==> _WATCHED_OFFSET mod 1 == 0 mod 1;
    ...
    v4$1 := (if p3$1 then BV_EXTRACT(BV64_ADD(v1$1, $c0.0$1), 32, 0) else v4$1);
    v4$2 := (if p3$2 then BV_EXTRACT(BV64_ADD(v1$2, $c0.0$2), 32, 0) else v4$2);
