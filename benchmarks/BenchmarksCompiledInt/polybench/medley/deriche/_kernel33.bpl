...
var {:source_name "tp2"} {:global} $$tp2: [int]int;
...
procedure {:source_name "kernel33"} {:kernel} $kernel33($h: int, $w: int, $c0: int);
  requires {:sourceloc_num 0} {:thread 1} (if $h == 256 then 1 else 0) != 0;
  requires {:sourceloc_num 1} {:thread 1} (if $w == 1024 then 1 else 0) != 0;
  requires {:sourceloc_num 2} {:thread 1} (if BV32_AND(BV32_AND(BV32_AND(BV32_AND(BV1_ZEXT32((if BV32_SLE($h, 2147483647) then 1 else 0)), BV1_ZEXT32((if BV32_SLE($w, 2147483647) then 1 else 0))), BV1_ZEXT32((if BV32_SGE($w, -2147483647) then 1 else 0))), BV1_ZEXT32((if BV64_SGE(BV32_SEXT64($h), BV64_ADD($c0, 1)) then 1 else 0))), BV1_ZEXT32((if BV64_SGE($c0, 0) then 1 else 0))) != 0 then 1 else 0) != 0;
...
implementation {:source_name "kernel33"} {:kernel} $kernel33($h: int, $w: int, $c0: int)
{

  $0:
    call {:sourceloc} {:sourceloc_num 4} _LOG_WRITE_$$tp2(true, 0, 0, $$tp2[0]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$tp2(true, 0);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 4} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 4} _CHECK_WRITE_$$tp2(true, 0, 0);
