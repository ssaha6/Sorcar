...
implementation {:source_name "kernel0"} {:kernel} $kernel0($n: int, $c0: int, $c1: int)
{
  ...
  $10:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b7 ==> _WRITE_HAS_OCCURRED_$$path ==> _WATCHED_OFFSET mod 1 == 0 mod 1;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b6 ==> _READ_HAS_OCCURRED_$$path ==> _WATCHED_OFFSET mod 1 == 0 mod 1 || _WATCHED_OFFSET mod 1 == 0 mod 1 || _WATCHED_OFFSET mod 1 == 0 mod 1 || _WATCHED_OFFSET mod 1 == 0 mod 1 || _WATCHED_OFFSET mod 1 == 0 mod 1 || _WATCHED_OFFSET mod 1 == 0 mod 1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b5 ==> BV64_SLE($c2.0$1, $3$1) ==> p6$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b5 ==> BV64_SLE($c2.0$2, $3$2) ==> p6$2;
    assert {:tag "loopBound"} {:thread 1} p6$1 ==> _b4 ==> BV32_UGE($c2.0$1, $1$1);
    assert {:tag "loopBound"} {:thread 2} p6$2 ==> _b4 ==> BV32_UGE($c2.0$2, $1$2);
    assert {:tag "loopBound"} {:thread 1} p6$1 ==> _b3 ==> BV32_ULE($c2.0$1, $1$1);
    assert {:tag "loopBound"} {:thread 2} p6$2 ==> _b3 ==> BV32_ULE($c2.0$2, $1$2);
    assert {:tag "loopBound"} {:thread 1} p6$1 ==> _b2 ==> BV32_SGE($c2.0$1, $1$1);
    assert {:tag "loopBound"} {:thread 2} p6$2 ==> _b2 ==> BV32_SGE($c2.0$2, $1$2);
    assert {:tag "loopBound"} {:thread 1} p6$1 ==> _b1 ==> BV32_SLE($c2.0$1, $1$1);
    assert {:tag "loopBound"} {:thread 2} p6$2 ==> _b1 ==> BV32_SLE($c2.0$2, $1$2);
    assert {:tag "loopCounterIsStrided"} {:thread 1} p6$1 ==> _b0 ==> $c2.0$1 mod 1048576 == $1$1 mod 1048576;
    assert {:tag "loopCounterIsStrided"} {:thread 2} p6$2 ==> _b0 ==> $c2.0$2 mod 1048576 == $1$2 mod 1048576;
