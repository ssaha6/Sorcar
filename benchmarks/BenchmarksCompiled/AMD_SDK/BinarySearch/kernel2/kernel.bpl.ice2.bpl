...
implementation {:source_name "binarySearch_mulkeys"} {:kernel} $binarySearch_mulkeys($numKeys: bv32)
{
  ...
  $0:
    ...
    assume BV1_ZEXT32((if BV32_SLT(v0$1, v1$1) ==> BV32_ULT($$input[v0$1], $$input[v1$1]) then 1bv1 else 0bv1)) != 0bv32 && BV1_ZEXT32((if BV32_SLT(v0$2, v1$2) ==> BV32_ULT($$input[v0$2], $$input[v1$2]) then 1bv1 else 0bv1)) != 0bv32;
    assume BV1_ZEXT32((if BV32_SLT(v1$2, v0$1) ==> BV32_ULT($$input[v1$2], $$input[v0$1]) then 1bv1 else 0bv1)) != 0bv32 && BV1_ZEXT32((if BV32_SLT(v1$1, v0$2) ==> BV32_ULT($$input[v1$1], $$input[v0$2]) then 1bv1 else 0bv1)) != 0bv32;
...
assert  my_inv (  (  BV32_SLE($i.0, 0bv32) ) ,  (  BV32_SGE($i.0, 0bv32) ) ,  (  BV32_ULE($i.0, 0bv32) ) ,  (  BV32_UGE($i.0, 0bv32) )  ); 


    assert {:block_sourceloc} {:sourceloc_num 2} true;
    assert {:originated_from_invariant} {:sourceloc_num 3} {:thread 1} (if _WRITE_HAS_OCCURRED_$$output ==> BV32_ULT(BV32_UDIV(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32), $i.0) then 1bv1 else 0bv1) != 0bv1;
