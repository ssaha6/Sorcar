...
var {:source_name "ym2"} {:global} $$ym2: [bv32]bv32;
...
procedure {:source_name "kernel26"} {:kernel} $kernel26($h: bv32, $w: bv32, $c0: bv64);
  ...
  requires {:sourceloc_num 2} {:thread 1} (if BV32_AND(BV32_AND(BV32_AND(BV32_AND(BV1_ZEXT32((if BV32_SLE($h, 2147483647bv32) then 1bv1 else 0bv1)), BV1_ZEXT32((if BV32_SLE($w, 2147483647bv32) then 1bv1 else 0bv1))), BV1_ZEXT32((if BV32_SGE($w, 2147483649bv32) then 1bv1 else 0bv1))), BV1_ZEXT32((if BV64_SGE(BV32_SEXT64($h), BV64_ADD($c0, 1bv64)) then 1bv1 else 0bv1))), BV1_ZEXT32((if BV64_SGE($c0, 0bv64) then 1bv1 else 0bv1))) != 0bv32 then 1bv1 else 0bv1) != 0bv1;
  ...
  modifies _WRITE_HAS_OCCURRED_$$ym2, _WRITE_READ_BENIGN_FLAG_$$ym2, _WRITE_READ_BENIGN_FLAG_$$ym2;



implementation {:source_name "kernel26"} {:kernel} $kernel26($h: bv32, $w: bv32, $c0: bv64)
{

  $0:
    call {:sourceloc} {:sourceloc_num 4} _LOG_WRITE_$$ym2(true, 0bv32, 0bv32, $$ym2[0bv32]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$ym2(true, 0bv32);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 4} true;
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 4} _CHECK_WRITE_$$ym2(true, 0bv32, 0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$ym2"} true;
    $$ym2[0bv32] := 0bv32;
    $$ym2[0bv32] := 0bv32;
    return;
}
