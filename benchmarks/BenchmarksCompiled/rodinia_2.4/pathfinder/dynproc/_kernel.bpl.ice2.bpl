...
implementation {:source_name "dynproc_kernel"} {:kernel} $dynproc_kernel($iteration: bv32, $cols: bv32, $rows: bv32, $startStep: bv32, $border: bv32, $HALO: bv32)
{
  ...
  __partitioned_block_$0_0:
    ...
    $0$1 := (if p0$1 then BV32_SUB(0bv32, v2$1) else $0$1);
    $0$2 := (if p0$2 then BV32_SUB(0bv32, v2$2) else $0$2);
    ...
    $1$1 := (if p2$1 then BV32_SUB(BV32_SUB(v0, 1bv32), BV32_ADD(BV32_SUB(v3$1, $cols), 1bv32)) else $1$1);
    $1$2 := (if p2$2 then BV32_SUB(BV32_SUB(v0, 1bv32), BV32_ADD(BV32_SUB(v3$2, $cols), 1bv32)) else $1$2);
    $1$1 := (if p3$1 then BV32_SUB(v0, 1bv32) else $1$1);
    $1$2 := (if p3$2 then BV32_SUB(v0, 1bv32) else $1$2);
    ...
    $4$1 := (if p8$1 then (if BV32_SLE(v1$1, $1$1) then 1bv1 else 0bv1) else $4$1);
    $4$2 := (if p8$2 then (if BV32_SLE(v1$2, $1$2) then 1bv1 else 0bv1) else $4$2);
    $4$1 := (if p9$1 then 0bv1 else $4$1);
    $4$2 := (if p9$2 then 0bv1 else $4$2);
...
assert  my_inv (  (  BV32_SLE(0bv32, $i.0) ) ,  (  BV32_SLE($i.0, 0bv32) ) ,  (  BV32_SGE($i.0, 0bv32) ) ,  (  BV32_ULE($i.0, 0bv32) ) ,  (  BV32_UGE($i.0, 0bv32) ) ,  (  !_WRITE_HAS_OCCURRED_$$outputBuffer ) ,  (  !_READ_HAS_OCCURRED_$$prev ) ,  (  !_WRITE_HAS_OCCURRED_$$prev ) ,  (  _READ_HAS_OCCURRED_$$prev ==> _WATCHED_OFFSET == local_id_x$1 ) ,  (  _WRITE_HAS_OCCURRED_$$prev ==> _WATCHED_OFFSET == local_id_x$1 ) ,  (  !_READ_HAS_OCCURRED_$$result ) ,  (  !_WRITE_HAS_OCCURRED_$$result ) ,  (  _READ_HAS_OCCURRED_$$result ==> _WATCHED_OFFSET == local_id_x$1 ) ,  (  _WRITE_HAS_OCCURRED_$$result ==> _WATCHED_OFFSET == local_id_x$1 )  ); 
  ...
  __partitioned_block_$truebb6_0:
    ...
    v19$1 := (if p17$1 then $4$1 == 1bv1 else v19$1);
    v19$2 := (if p17$2 then $4$2 == 1bv1 else v19$2);
