...
implementation {:source_name "kmeans_swap"} {:kernel} $kmeans_swap($npoints: bv32, $nfeatures: bv32)
{
  ...
  var _WRITE_HAS_OCCURRED_$$feature_swap$ghost$$for.cond: bool;
  ...
  $entry:
    ...
    _WRITE_HAS_OCCURRED_$$feature_swap$ghost$$for.cond := _WRITE_HAS_OCCURRED_$$feature_swap;
    assume {:captureState "loop_entry_state_0_0"} true;
    goto $for.cond;

  $for.cond:
    assume {:captureState "loop_head_state_0"} true;
    assert {:tag "disabledMaintainsInstrumentation"} _b12 ==> !p0$1 ==> _WRITE_HAS_OCCURRED_$$feature_swap$ghost$$for.cond == _WRITE_HAS_OCCURRED_$$feature_swap;
    assert {:tag "accessBreak"} _b11 ==> _WRITE_HAS_OCCURRED_$$feature_swap ==> group_id_x$1 == BV32_DIV(_WATCHED_OFFSET, group_size_x);
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b10 ==> _WRITE_HAS_OCCURRED_$$feature_swap ==> BV32_AND(BV32_SUB(BV32_MUL(1bv32, $npoints), 1bv32), _WATCHED_OFFSET) == BV32_AND(BV32_SUB(BV32_MUL(1bv32, $npoints), 1bv32), BV32_ADD(BV32_MUL(0bv32, $npoints), BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)));
    assert {:tag "accessUpperBoundBlock"} _b9 ==> _WRITE_HAS_OCCURRED_$$feature_swap ==> BV32_SLT(_WATCHED_OFFSET, BV32_ADD(BV32_MUL(BV32_ADD(group_id_x$1, 1bv32), group_size_x), local_id_x$1));
    assert {:tag "accessLowerBoundBlock"} _b8 ==> _WRITE_HAS_OCCURRED_$$feature_swap ==> BV32_SLE(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), _WATCHED_OFFSET);
    assume {:predicate "p1"} {:dominator_predicate "p0"} true;
    assert {:do_not_predicate} {:tag "accessOnlyIfEnabledInEnclosingScopes"} {:thread 1} _b7 ==> _WRITE_HAS_OCCURRED_$$feature_swap ==> BV32_ULT(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), $npoints);
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b6 ==> BV32_ULT(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), $npoints) && BV32_SLT($i.0$1, $nfeatures) ==> p1$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b6 ==> BV32_ULT(BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2), $npoints) && BV32_SLT($i.0$2, $nfeatures) ==> p1$2;
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 1} p1$1 ==> _b5 ==> p1$1 ==> BV32_ULT(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), $npoints);
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 2} p1$2 ==> _b5 ==> p1$2 ==> BV32_ULT(BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2), $npoints);
