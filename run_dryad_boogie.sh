...
# call using: sh run_dryad.sh "-a sorcar -f -t -r" 
# SORCAR_OPTION=$1
...
# ------------------------------------------------- #
PREFIX=`pwd`
export LD_LIBRARY_PATH=$PREFIX/z3/bin:$LD_LIBRARY_PATH
...
SOURCELIST="$PREFIX/benchmarks/BenchmarksDryad/phase3bplfiles"
...
# *) 
    (
        time  /usr/bin/timeout $TIMEOUT mono ./Boogie.exe  $USUAL_OPT /z3exe:$Z3_BIN  $BOOGIE_OPT $PRELUDEFILE $WORK_DIR/$file ;
    ) 
